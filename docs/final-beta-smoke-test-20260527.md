...
**실 환경 최종 검증 (OZ, 2026-06-02, 기준 commit `64e2140`) — 판정: PASS, 계정 복구 / 비밀번호 재설정 P1 종료**
- 학부모/학생 로그인 화면 "비밀번호를 잊으셨나요?" 링크 노출 확인
- `/auth/forgot-password` 진입 + 가입 이메일 입력 후 재설정 메일 발송·수신 확인
- 메일 링크 클릭 → `/auth/reset-password` 진입 확인
- 새 비밀번호 **변경 성공** 확인
- 변경된 새 비밀번호로 **정상 로그인** 확인
- → 계정 복구 / 비밀번호 재설정 P1 이슈 종료 (updateUser 실패 포함)
...
| 2026-06-03 | 고용24 seed 콘텐츠 점검 및 직업 상세 확장 2단계 문서(`docs/go24-seed-content-audit-and-detail-expansion-plan.md`) 생성 — job_satisfaction은 seed 다수 보유(P1 즉시), traits/parent_questions/related_jobs/source_meta는 seed 0건(보류). 코드/UI/DB 변경 없음. | — |
| 2026-06-03 | (3단계 P1) 직업 상세 "미래를 그리는 참고 지표"에 직업만족도 1줄 추가(`Goyo24InfoSection`). job_satisfaction 정상값 "N점"만 표시, 없으면 미표시. 기존 임금/전망/관련학과 유지. DB/migration/API 변경 없음. | 5ebc758 |
...
| 2026-06-03 | 자녀 추가/삭제(비활성) 관리 기능 추가 — `/settings/children` 신규(active 목록·경량 추가 폼·삭제 모달), `/settings`에 진입 카드. child_limit DB 기준 준수(초과 시 추가 차단 안내), 삭제는 soft delete(`profile_status='inactive'`, 하드 삭제/cascade 없음). FAQ 9번 실기능 기준 보정. DB schema/migration/RLS 변경 없음. 다자녀 전환 selector는 후속. | — |
