...
| 항목 | 현재 | 수정 방향 | 상태 |
|---|---|---|---|
| `/pricing` 카드 순서 | 유료 4개 → 무료 체험 (맨 아래) | **무료 체험 박스를 맨 위로** 이동 | ❌ 대기 |
| `/pricing` 유료 CTA 버튼 | "내 아이 진로 지도 만들기" (클릭 시 alert) | "정식 오픈 시 신청하기" | ❌ 대기 |
| `/guide` step 9 | 실가격 직접 노출 (9,900원~24,900원) | 베타 무료 안내로 교체, 가격 제거 | ❌ 대기 |
| `/faq` "결제 후 이용 가능?" | "결제 완료 즉시 모든 유료 기능 이용 가능" (현실 불일치) | 베타 준비 중 안내로 수정 | ❌ 대기 |
| `/faq` "무료 이용?" | "최초 가입 시 14일 무료 체험" (정책 불일치) | 베타 기간 무료 안내로 수정 | ❌ 대기 |
...
| 항목 | 현재 | 수정 방향 | 상태 |
|---|---|---|---|
| 퀴즈 진입 전 맥락 표시 | 없음 | "👧 아이와 함께 풀어보는 직업 퀴즈예요" 안내 추가 | ❌ 대기 |
| 퀴즈 결과 카드 하단 | "다시 도전하기" 버튼만 존재 | "아래 '로드맵 만들기' 버튼으로 다음 단계를 시작해봐요 👇" 보조 문구 추가 | ❌ 대기 |
| MissionSuccessModal compass sub | "네 꿈이 한 칸 더 가까워졌어." | "퀴즈를 마쳤어요. 이 직업으로 로드맵 만들기를 눌러 다음 단계로 가봐요!" | ❌ 대기 |
...
※ 명따라 외부 단건 판매: 9,900원/회 (좋소아빠 채널) · 꿈따라 스탠다드 이상: 5,000원
...
| 3주차 (5/15~5/21) | 플랜별 기능 분기 · 구독 해지·환불 로직 · 보호자 초대 코드 | 🔨 진행 중 |
...
> 꿈따라 · 꿈을 찾고, 길을 만든다
> 꿈따라.kr | 좋소아빠 대표 OZ.Kim | johsoappa@gmail.com
> 운영시간: 평일 19:00~22:00
